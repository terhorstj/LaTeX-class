...
Once you run ```mktexlsr```, TeXLive will add them to your path.
...
* ```epx``` - use ```newpxtext``` and ```eulerpx``` fonts
...
* ```wide``` - wide (1.8 inch) left/right margins via ```geometry```
* ```narrow``` - narrow (0.5 inch) margins via ```geometry```
* ```ars``` - restyle sections in an Ars Classica theme
* ```blockpar``` - use block paragraphs via ```parskip```
* ```onehalf``` - one-and-a-half spacing via ```setspace``` with ```\onehalfspacing```
* ```double``` - double spacing via ```setspace``` with ```\doublespacing```
* ```nolinespacing``` - revert font-based linespacing adjustments to use 1.0 linespacing
...
* ```lists``` or ```tightlists``` or ```looselists``` - tweak spacing in default itemized/enumerated lists
* ```misc``` - extra useful packages: graphicx, xcolor, float, nicefrac, siunitx, recycle, and nowidow.
* ```tagging``` - include package settings for accessibility tagging
...
This option uses the expanded Latin Modern fonts via ```lmodern```. The following supporting packages are loaded: 
...
* ```\RequirePackage{setspace}\setstretch{1.1}``` - give linespacing a little extra breathing room. Stretch can be readjusted in the preamble.
* ```\RequirePackage{biolinum}``` - Biolinum sans serif font
* ```\RequirePackage{upgreek}``` - load matching upright Greek in math mode; ```\text``` aliases follow.
...
* ```\RequirePackage[scr=boondox,scrscaled=1.1]{mathalpha}``` - load Boondox script
...
* ```\RequirePackage[scr=boondox,scrscaled=1.0]{mathalpha}``` - load Boondox script
* ```\RequirePackage{biolinum}``` - Biolinum sans serif font
...
Example: ```\documentclass[10pt,epx]{jpt}```

This option uses Palatino clone fonts from ```newpxtext``` with Euler math from ```eulerpx```. Here I also include some preferred options and supporting packages. Linespacing is expanded slightly more than what is used for other fonts, and matching upright Greek alphabets are included from the Euler family via ```textgreek```. The matching calligraphic E provided by the ```emf``` package.
...
* ```\RequirePackage{setspace}\setstretch{1.1}``` - adjust linespacing 
* ```\RequirePackage{textgreek}``` - load matching upright Greek in text mode
* ```\RequirePackage[scr=cm,scrscaled=0.99]{mathalpha}``` - load matching math script
...
* ```\RequirePackage{setspace}\setstretch{1.1}``` - adjust linespacing
* ```\RequirePackage[artemisia]{textgreek}``` - load matching upright Greek in text mode
* ```\RequirePackage[scr=txupr,cal=txupr,scrscaled=0.925,calscaled=0.925]{mathalpha}``` - load matching calligraphic and math script
...
* ```\RequirePackage[artemisia]{textgreek}``` - load matching upright Greek in text mode
* ```\RequirePackage[scr=boondox,scrscaled=1.05]{mathalpha}``` - load matching calligraphic and math script 
* ```\RequirePackage{biolinum}``` - Biolinum sans serif font
* ```\RequirePackage{setspace}\setstretch{1.1}``` - adjust linespacing
...
This option uses the Linux Libertine font with math support from ```newtxmath```. Other supporting packages are included to provide matching upright Greek alphabets, and to provide a matching ```\emf``` character, scaled to match the Libertine font. (Note: ```newtxmath``` defines its own "upgreek"-style glyphs (```$\upalpha$```, ```$\upbeta$```, etc.) to match ```Libertine```, so the practical use of ```textgreek``` is mostly for compatibility.)
...
Example: ```\documentclass[10pt,cm,wide]{jpt}```

This option implements the ```geometry``` package and sets top/bottom margins to 1 inch and wide left/right margins to 1.8 inch; this gives a narrow textwidth dimension for more ideal line widths. The ```geometry``` package algorithmically redefines headers, footers, and marginnote spacing based on margin availability.
...
Example: ```\documentclass[10pt,cm,onehalf]{jpt}```	

This option implements one-and-a-half spacing via ```setspace``` with ```\onehalfspacing```. This option overrides any other line spacing set by the font choice. If both ```onehalf``` and ```double``` (below) are invoked, ```double``` will override.

### Double

Example: ```\documentclass[10pt,cm,double]{jpt}```	

This option implements double spacing via ```setspace``` with ```\doublespacing```. This option overrides any other line spacing set by the font choice. If both ```onehalf``` (above) and ```double``` are invoked, ```double``` will override.
...
It also includes a new ```\blfootnote``` command (blank footnote) which creates a footnote with no cross-reference number.
...
Example: ```\documentclass[10pt,npx,wide,ars,footnotes,tables,captions]{jpt}```	
...
This option can be used in conjunction with any other global options; however the original Ars Classica document "look" can be best achieved by combining the options ```[10pt,npx,wide,ars,footnotes,tables,captions]```.
...
* ```\newcommand{\da}[4]{\left(\frac{#1\myu{#2}}{#3\myu{#4}}\right)}```
...
* ```\RequirePackage[shortlabels]{enumitem}\setlist{noitemsep}``` 

### Tightlists

Example: ```\documentclass[10pt,cm,tightlists]{jpt}```
...
Example: ```\documentclass[10pt,cm,looselists]{jpt}```

Use this option to only enable ```enumitem``` with ```shortlabels```.
...
Example: ```\documentclass[10pt,lm,tagging]{jpt}```

Uses ```\RequirePackage{tagpdf}\tagpdfsetup{activate-all}\tagpdfsetup{math/alt/use}```

Documents must also include ```\DocumentMetadata{lang = en-US,pdfversion = 2.0,pdfstandard = ua-2,tagging=on,tagging-setup={math/setup=mathml-SE}}``` at the very top (before document class declaration) and document must be compiled with LuaLaTeX to include MathML in the PDF. Figures and tables must be tagged in the document body (templates are in TextExpander).
